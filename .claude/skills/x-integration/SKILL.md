---
name: x-integration
description: X (Twitter) integration for OpenNekaise. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** OpenNekaise v1.0.0. Directory structure may change in future versions.
...
1. **OpenNekaise is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep opennekaise (macOS) or systemctl --user status opennekaise (Linux)
```
...
| `logs/opennekaise.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into OpenNekaise, make the following modifications:
...
**Verify success:**
```bash
launchctl list | grep opennekaise  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status opennekaise
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/opennekaise.log | tail -20

# Script errors
grep -i "error\|failed" logs/opennekaise.log | tail -20
```
...
docker run opennekaise ls -la /app/src/skills/
```
