---
name: update
description: "Update OpenNekaise from upstream. Fetches latest changes, merges with your customizations and skills, runs migrations. Triggers on \"update\", \"pull upstream\", \"sync with upstream\", \"get latest changes\"."
---
...
```bash
test -d .opennekaise && echo "INITIALIZED" || echo "NOT_INITIALIZED"
```
...
**No upstream remote:** The fetch script auto-adds `upstream` pointing to `https://github.com/qwibitai/opennekaise.git`. If the user forked from a different URL, they should set the remote manually: `git remote add upstream <url>`.
...
**Rollback:** If something goes wrong after applying but before cleanup, the backup is still in `.opennekaise/backup/`. Run:
```bash
