...
# Fetch upstream OpenNekaise and extract to a temp directory.
...
if git remote get-url upstream &>/dev/null; then
  ...
  if echo "$ORIGIN_URL" | grep -q "qwibitai/opennekaise"; then
    REMOTE="origin"
  fi
fi

if [ -z "$REMOTE" ]; then
  echo "No upstream remote found. Adding upstream → https://github.com/qwibitai/opennekaise.git"
  git remote add upstream https://github.com/qwibitai/opennekaise.git
  REMOTE="upstream"
fi
...
TEMP_DIR=$(mktemp -d /tmp/opennekaise-update-XXXX)
