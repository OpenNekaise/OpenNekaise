...
- Added: `gmail` MCP server alongside the existing `opennekaise` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `opennekaise` MCP server configuration is unchanged
...
## Must-keep
- The `opennekaise` MCP server with its environment variables
