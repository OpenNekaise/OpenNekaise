...
## Key sections
- **readEnvFile call**: Must include `SLACK_ONLY` in the keys array. OpenNekaise does NOT load `.env` into `process.env` — all `.env` values must be explicitly requested via `readEnvFile()`.
