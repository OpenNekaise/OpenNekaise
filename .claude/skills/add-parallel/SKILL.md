...
Adds Parallel AI MCP integration to OpenNekaise for advanced web research capabilities.
...
- **Non-blocking Design** - Uses OpenNekaise scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. OpenNekaise already set up and running
...
Add Parallel AI MCP servers after the opennekaise server:
```typescript
const mcpServers: Record<string, any> = {
  opennekaise: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__opennekaise__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__opennekaise__send_message
   4. Use mcp__opennekaise__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo opennekaise:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep opennekaise  # macOS
...
- Check task runs: `tail -f logs/opennekaise.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.opennekaise` (macOS) or `systemctl --user restart opennekaise` (Linux)
