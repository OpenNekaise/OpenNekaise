...
This skill switches OpenNekaise's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
Read `.opennekaise/state.yaml`. If `convert-to-apple-container` is in `applied_skills`, skip to Phase 3 (Verify). The code changes are already in place.
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo opennekaise:latest "Container OK"
```
