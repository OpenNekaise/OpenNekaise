...
This skill adds Telegram support to OpenNekaise using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.opennekaise/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep opennekaise` (macOS) or `systemctl --user status opennekaise` (Linux)
...
- Check bot is started: `tail -f logs/opennekaise.log`
...
8. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.opennekaise` (macOS) or `npm run build && systemctl --user restart opennekaise` (Linux)
