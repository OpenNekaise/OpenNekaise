...
export function initSkillsSystem(): void {
  initNanoclawDir();
  console.log('Skills system initialized. .opennekaise/ directory created.');
}
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      const filesModified = [...diff.matchAll(/^diff -ruN .+ (.+)$/gm)]
        ...
        .filter((f) => !f.startsWith('.opennekaise'));
      ...
      console.log(
        'Custom modifications captured in .opennekaise/custom/migration.patch',
      );
    } else {
