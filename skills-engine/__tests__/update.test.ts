...
describe('update', () => {
  ...
  function writeStateFile(state: Record<string, unknown>): void {
    const statePath = path.join(tmpDir, '.opennekaise', 'state.yaml');
        ...
        custom_modifications: [
          {
            ...
            patch_file: '.opennekaise/custom/001-tweak.patch',
          },
        ],
      });
  ...
  describe('applyUpdate', () => {
    it('rejects when customize session is active', async () => {
      ...
      const customDir = path.join(tmpDir, '.opennekaise', 'custom');
