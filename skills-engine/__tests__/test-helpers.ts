...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'opennekaise-test-'));
}

export function setupNanoclawDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.opennekaise', 'base', 'src'), {
    recursive: true,
  });
  fs.mkdirSync(path.join(tmpDir, '.opennekaise', 'backup'), { recursive: true });
}
