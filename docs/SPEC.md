...
│  │    • mcp__opennekaise__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── opennekaise.log               # Host stdout
│   └── opennekaise.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.opennekaise.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'opennekaise:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your opennekaise installation
...
OpenNekaise uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: opennekaise (scheduler)
   │
   ▼
...
OpenNekaise has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__opennekaise__schedule_task]
        {
...
### OpenNekaise MCP (built-in)

The `opennekaise` MCP server is created dynamically per agent call with the current group's context.
...
OpenNekaise runs as a single macOS launchd service.

### Startup Sequence

When OpenNekaise starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned OpenNekaise containers from previous runs
...
### Service: com.opennekaise

**launchd/com.opennekaise.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/opennekaise.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/opennekaise.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.opennekaise.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.opennekaise.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.opennekaise.plist

# Check status
launchctl list | grep opennekaise
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep opennekaise` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; OpenNekaise auto-starts container runtime but may fail |
...
- `logs/opennekaise.log` - stdout
- `logs/opennekaise.error.log` - stderr
